...
- `deno task propose --vector <vector>`
...
- Proposal generation writes `docs/generated/improvement/<vector>-proposal.json`.
- Failing or regressing scores can scaffold `changes/<change-id>/`.
...
- Re-run `deno task propose --vector <vector>` after refreshing the score artifact.
